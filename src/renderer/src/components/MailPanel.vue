  ...
  <section class="mail-panel">
    ...
    <div v-if="!store.configured" class="login-area">
      <div class="login-card">
        ...
        <form @submit.prevent="handleConfig" class="mail-form">
          <label class="field"><span>Exchange 服务器</span><input v-model="form.server" placeholder="mail.company.com" required @input="scheduleSaveMailIdentity"></label>
          ...
          <label class="field"><span>AD 域账号</span><input v-model="form.domainUser" placeholder="zhang.wei" required @input="scheduleSaveMailIdentity"></label>
    ...
    <div v-else style="flex:1;min-height:0;display:flex;flex-direction:column">
      ...
      <div v-else class="empty-state">
        <Mail :size="30" /><p>收件箱为空</p>
        <button class="save-btn" style="margin-top:8px" @click="handleFetch">拉取邮件</button>
      </div>
    </div>
...
const form = ref({ server: '', email: '', domainUser: '', domain: DEFAULT_MAIL_DOMAIN, password: '' })
...
onMounted(async () => {
  ...
  if (saved?.mailConfig) {
    form.value = {
      server: saved.mailConfig.server || '',
...
async function saveMailIdentity() {
  ...
  await window.api?.settings?.save({
    mailConfig: {
      ...
      domainUser: form.value.domainUser || '',
      domain: form.value.domain || DEFAULT_MAIL_DOMAIN,
      password: ''
    }
  })
}
